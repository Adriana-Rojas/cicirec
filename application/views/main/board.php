            ...
            ?>
						<tr>
							...
							<td class="text-center"><a
								href="<?= base_url()?>OrdersAppOrder/tracerProcess/<?= $this->encryption->encrypt ( $value->HISTORIA );?>/<?= $this->encryption->encrypt ( $value->ID );?>"
...
//-->
	new Morris.Bar({
	  ...
	  // the chart.
	  data: [
	    { tipo: '<?= $tipo1?>', value: <?= $valor1?> },
	    { tipo: '<?= $tipo2?>', value: <?= $valor2?> },
	    <?php if ($tipo3!=''){?>{ tipo: '<?= $tipo3?>', value: <?= $valor3?> },<?php }?>
	    <?php if ($tipo4!=''){?>{ tipo: '<?= $tipo4?>', value: <?= $valor4?> },<?php }?>
	    <?php if ($tipo5!=''){?>{ tipo: '<?= $tipo5?>', value: <?= $valor5?> },<?php }?>
	  ],
